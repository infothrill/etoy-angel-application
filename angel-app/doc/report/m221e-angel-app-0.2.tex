...
\mbox{}\\ 
\mbox{}\\
Eternal Storage in Social Networks: MISSION ETERNITYÕs \textsc{Angel-Application}}
\author{}
...
\begin{abstract}
Current storage media such as hard disks, CD or DVD ROMs suffer from very limited average lifetimes on the order of 2-10 years. One way to increase data lifetime is to increase the lifetime of the storage medium used. Another approach is to employ a pervasive and consistent backup policy: \emph{i.e.} to store redundant copies of the data, and continuously validate the resulting redundant data set, replacing corrupted pieces of data by correct ones.
The latter approach is particularly interesting, because it can be implemented using a set of networked standard desktop computers. 

In this article, we discuss such an approach, in which users collaborate to back up each other's data via a fully autonomous peer-to-peer protocol. Its key design features are
\begin{itemize}
\item absence of a  single point of failure (assuming no breakdown of current cryptographic systems);
\item in the absence of cooperative peers, the system's behavior degenerates to that of a normal local filesystem. It is therefore byzantine fail-safe.
\item The internal representation of the system is hidden from the user by exposing a standard file-system interface (tree traversal, node and branch manipulation) via a commonly supported network file system protocol (WebDAV for the time being).
\end{itemize}
We believe the present approach is attractive, because (i) it should in general allow data lifetimes to scale exponentially with the physical storage space used (corresponding to predicted lifetimes exceeding that of the universe for common cases), (ii) apart from an initial setup, and optional fine-tuning, the approach is completely transparent to day-to-day filesystem use, (iii) no resources beyond spare hard-disk space, CPU time and network bandwidth are used,  (iv) the system capacity is capable of growing in line with system requirements, (v) reasonably strong privacy and security guarantees can be made for the data injected into the system. We discuss the underlying principles of such a system as well as practical problems encountered in the implementation of a functional prototype.
\end{abstract}

\newpage
\noindent Version 0.2.1, July 2008: Vincent Kr\"autler and Paul Kremer, etoy.VENTURE association.

\mbox{}\\

\noindent \copyright \mbox{} 2005-2008, etoy.VENTURE association. This document is available under a creative commons license as described at: \texttt{http://creativecommons.org/licenses/by-nc-nd/3.0/}.

\vspace{10cm}
Following page: Visualization of the module import graph of the core (``maintainer'') process of the \textsc{Angel-Application}. This process negotiates inter-node communication, keeps track of resource replicas and verifies and replicates the resource graph.
\thispagestyle{nonumber}

\newpage
\begin{center}
\includegraphics[height=\textheight]{figures/eps/maintainer.pdf}
\end{center}
...
Firstly, the deterioration of data is not completely stopped, but only slowed down to the characteristic lifetime of the new storage medium. The second important drawback is that the corresponding storage media by their very nature tend to be rather static -- it is difficult to edit a hieroglyph that has been chiseled into stone. So while degradation is slowed down, the cost of reversing partial degradation is correspondingly higher. The data is effectively frozen.

A complementary approach uses the fact that entropy does not necessarily increase (\emph{i.e.} information is not necessarily lost) in open systems (\emph{i.e.} those which exchange energy with their environment). Such systems in principle allow for information to be stored indefinitely, in fact for as long as a suitable coupling to an external environment exists.  An impressive and particularly extreme example of such a system are cyanobacteria: the information defining that species is stored in a single molecule of DNA (an extremely fragile medium) but has nevertheless been largely preserved for billions of years via a replication mechanism. The main drawback of such systems is the question of resource allocation -- deciding which piece of information gets to be replicated and where the result is stored. This biological example is therefore not directly transferrable to most current data storage approaches, since it will indiscriminately use all available resources and does not (strongly) discriminate against mutation. 

A backup system aims to obtain the best of both approaches, combining (i) a replication mechanism which discards undesirable replica, and (ii) reliable storage media. Sophisticated backup systems typically employ a hierarchical structure, with backups taken at increasingly large intervals (daily, weekly, monthly), and stored in increasingly safe, inaccessible, and physically separated locations, with exact preservation of the original data being in principle possible through the use of checksumming and error correcting algorithms made possible due to the exact (digital) representation of the information in question. However, such procedures frequently carry a significant infrastructure and administrative cost. Sophisticated but easy to use backup systems are therefore often only accessible to large, well-funded organizations, resulting in a situation where most people do not backup their data regularly and properly.
...
\end{equation}
are still working. This formula is based on the assumption of spontaneous (uncorrelated) and irreversible failure, similar to \emph{e.g.} radioactive decay. The corresponding differential form illustrates that the rate of decay is assumed to be proportional to the (time-dependent) existing population
\begin{equation}
...
Now consider a system consisting of a collection of $M$ storage media with identical content, where at the end of every day (\emph{i.e.} after $\Delta t$), all bad media are discarded and replaced by copies of a good medium. So in order for the data to be lost, all storage media must fail during the course of one single day. Again using the above assumption of uncorrelated probabilities (\emph{i.e.} the probability of failure of one storage medium is independent of whether another storage medium has failed), the total probability of all media failing can be written as the product of the probabilities of the individual media failing, namely
\begin{equation}
...
\label{destroyBig}
\end{equation}
Comparing Eqs. (\ref{destroyBig}) and (\ref{destroySmall}), we find that we have built a new system for which we can identify a new effective lifetime $\Theta$, with the parts of the system still having individual lifetimes of $\tau$. For readily attainable cases, such as $M = 5$ hard disks, a lifetime $\tau$ of 3 years and an inspection interval $\Delta t$ of one day, we find an effective lifetime $\Theta$ of $4.3^{12}$ years -- about  300 times the current estimated age of the universe. Clearly then, failure conditions other than media corruption will determine the lifetime of such a system.
...
Furthermore, it can be used as the starting point of a (rigorously, if necessary) byzantine fail-safe distributed storage system\footnote{Byzantine failure, named after a historical anecdote, is the failure of a system in the face of conspiracy or misinformation.}: 

a) Consider the case of a non-distributed, locally stored file system on a single-user node. All data is available, no data is shared, no information exchanged. Clearly, such a system is byzantine fail-safe, since the behavior of the system is entirely determined by local information and the user herself. 

b) Next, consider a generalization of such a system, in which all data is made publicly  available in read-only (and possibly encrypted) form. Such a system is still byzantine fail-safe (because the user does not depend on information supplied by her peers), and could, in principle, allow for the (unstructured) replication of data. 

c) Finally, let us allow for a small set of verifiable messages that nodes can send to each other. Where we define verifiable to mean that the correctness of the information contained in the message can be verified by the recipient. By definition, the resulting system is still byzantine fail-safe. Conspiracy will, in the worst case, lead to the absence of meaningful replication, since by a), we require the local storage of all relevant data.

The following sections aim to describe a system that takes these standard file system semantics as a starting point (and limiting case, when no network of benevolent peers is available), enhanced with the possibility of structured data replication (based on verifiable messages and contracts), suitable for use in a hostile network.

\subsection{Maintaining a Single Resource}
\label{maintain}

Let us first consider a single, locally stored, resource. \emph{A priori}, we have no way of finding out if the given resource is ``correct'' in the sense that the data stored in the resource is the same as when the resource was last written to. What can we do about this? Well, when we create or modify that resource, we can calculate a cryptographic checksum of the data stored in the resource, and store that checksum in the resource's metadata. This helps, because cryptographic checksums such as MD5 or SHA have the property that given two pieces of data $A$ and $B$, the corresponding checksums $c(A)$ and $c(B)$ will be identical if $A$ and $B$ are identical, but very different in all but a vanishingly small number of cases, even if $A$ and $B$ differ only slightly. Additionally, it is at present essentially impossible to come up with a $B$ such that $c(B)$ is identical to $c(A)$. So, given $A$ and the checksum of $c(A, t)$ computed at some time $t$, we can say with certainty whether $A$ has changed since $t$, by recalculating the checksum from $A$ and comparing it with $c(A,t)$.
...
Concluding this section, we can already identify the following properties of the resulting system:
\begin{itemize}
\item The resources stored in such a network may easily be private in the sense that their content may be encrypted using state-of-the art cryptographic technology, though of course one can not say how for long current technologies will remain secure. However, privacy is difficult to guarantee for metadata such as resource size.
...
\begin{figure}[ht!]
\begin{center}
\includegraphics[scale=0.9]{figures/eps/fileSystem.pdf}
\end{center}
\caption{
...
\begin{figure}[ht!]
\begin{center}
\includegraphics[scale=0.9]{figures/eps/mount.pdf}
\end{center}
\caption{
...
\begin{verbatim}
type MetaData = (UnsignedMetaData, SignedMetaData)
type ResourceOwner = PublicKeyString
type PublicKeyString = String

type UnsignedMetaData = [Field]
type SignedMetaData = (RequiredMetaData, [SignedField], [SignedFields])
type SignedField = (Signature, Field)
type SignedFields = (Signature, [Field])
...
\begin{verbatim}
data RequiredMetaData = RequiredMetaData Signature ResourceID ResourceOwner
                             ResourceRevision ResourceChildren ResourceEncrypted
\end{verbatim} \label{revision}
Metadata fields corresponding to partial signatures (\texttt{SignedField}) or intermediary checksums (\texttt{SignedFields}) may be introduced (\emph{e.g.} for performance or simplicity reasons), as long as a checksum of the \texttt{SignedMetaData} is produced which cannot be produced without the secret key corresponding to \texttt{ResourceOwner}.
\texttt{ResourceChildren} specify zero or more child resources
\begin{verbatim}
type ResourceChildren = [ResourceChild]
type ResourceChild = (ResourceID, ResourceOwner)
\end{verbatim}
A \texttt{ResourceChild} is therefore identified by  the identifier of the child resource, and \texttt{child-owner }is the public key (or a checksum thereof) of the child resource. \texttt{ResourceID}, and \texttt{ResourceRevision} (for reasons we will explore in the following sections) are a union of a file path with a date, and a natural number ($\in \mathbb N_0$), respectively:
\begin{verbatim}
type ResourceID = (FilePath, Date)
type ResourceRevision = Int
\end{verbatim} 
\texttt{resource-encrypted} is a boolean flag indicating whether the resource contents are encrypted
\begin{verbatim}
type ResourceEncrypted = Bool
\end{verbatim}
We can now validate a resource using functions of the type
\begin{verbatim}
validateMetaData :: SignedMetaData -> Bool
validateContent :: SignedField -> Bool
validateResource :: Content -> SignedMetaData -> Bool
\end{verbatim}
For reproducibility reasons, we require than an external representations of these data types  UTF-8 strings (XML) is available. When a resource is replicated inside the angel-application, the \texttt{RequiredMetaData} must be completely replicated, otherwise the resulting clone is invalid. This latter property implies that resource replication is a transactional process -- it either succeeds completely, or results in a partial replication, which can be identified and discarded. \label{transaction}
...
\begin{verbatim}
type ResourceClones = [Clone]
type ChildClones = ResourceClones
type Clone = URL
\end{verbatim}
For simplicity reasons, the current prototype of the \textsc{Angel-Application} assumes that the \texttt{ChildClones} may be inferred from the \texttt{ResourceClones}, \emph{i.e.} assumes that if a remote clone exists for a local clone, then for a local child clone of the resource a remote child clone may be found for the remote clone.
...
def graphWalker(node, getChildren, toEvaluate, backPack = None):
    ...
    for child in getChildren(node):
        for result in graphWalker(child, getChildren, toEvaluate, rr[1]):
            yield result
\end{verbatim}
where we use \texttt{backPack} to implement an optional maximum recursion depth. \texttt{getChildren} returns the URL's of the resource's \texttt{ResourceChildren} (if any), and \texttt{toEvaluate} may be written as:
\begin{verbatim}
...
\section{Resource Replication in a Dynamic, Hostile Environment}
\label{dynamic}
...
\item Clone replication is entirely based on \emph{verifyable} claims made by the peer node (all relevant data fields are signed, the signatures can be verified). A system such as the above extended with a replication mechanism is therefore also byzantine fault tolerant.
...
\begin{figure}[ht!]
\begin{center}
\includegraphics[scale=0.7]{figures/eps/webdav.pdf}
\end{center}
...
The above \emph{push} mechanism can be safely implemented as follows. We allow for the ability to send the signed metadata of a resource to a given node (implemented as a WebDAV \texttt{PROPPATCH} method). Being signed, this metadata message is verifiable. Furthermore, we require that a resource may be created on a remote node, if the parent resource contains a (signed) \texttt{ResourceChild} link to a resource that does not exist, or whose contents do not match its \texttt{Signature}. The push mechanism then consists of the following steps:
\begin{enumerate}
...
\bibitem{svn} The Subversion Version Control System, available online.
